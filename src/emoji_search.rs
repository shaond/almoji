...
pub fn search_emojis(query: &str, limit: usize) -> Vec<(String, &'static emojis::Emoji)> {
...
pub fn apply_skin_tone(emoji: &emojis::Emoji, skin_tone: &SkinTone) -> String {
...
pub fn apply_gender(emoji_str: &str, _gender: &Gender) -> String {
    // Return emoji as-is since gendered variants are already in the database
    emoji_str.to_string()
}

/// Get the emoji string from a search result, handling raw emoji markers
pub fn get_emoji_string(keyword: &str, emoji: &emojis::Emoji) -> String {
    if keyword.starts_with("__raw__:") {
        // Extract the raw emoji string from the marker
        keyword.strip_prefix("__raw__:").unwrap_or(emoji.as_str()).to_string()
    } else {
